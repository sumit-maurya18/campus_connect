...
// Server Component — GET /api/jobs/featured?limit=3
// Fields: title, organization, company, city, work_style, salary, experience, is_verified, source

import Link from "next/link";
import { MapPin, Briefcase, Clock, ArrowRight, BadgeCheck } from "lucide-react";
import SectionHeader from "@/components/common/SectionHeader";
import { getFeaturedJobs, type Job } from "@/lib/api/opportunities";
...
function logoColor(i: number) { return LOGO_COLORS[i % LOGO_COLORS.length]; }
function initials(name: string) {
  return name.split(/\s+/).slice(0, 2).map((w) => w[0]).join("").toUpperCase();
}
function displayName(item: Job): string {
  return item.company || item.organization || "Unknown";
}
function locationLabel(item: Job): string {
...
export default async function JobsSection() {
  let jobs: Job[] = [];
  try {
    jobs = await getFeaturedJobs(3);
  } catch (err) {
    console.error("JobsSection fetch failed:", err);
  }
